...
final class PersianValue
{
    ...
    /**
     * پلاک به قالب رسمی ایران: دو رقم، حرف، سه رقم، «ایران»، دو رقم کد استان
     * — مثل «۱۲ ب ۳۴۵ ایران ۶۷». همان چیزی که generate_plate_number موتور
     * می‌سازد و format_plate در app/ocr/vehicle_card_ocr.py برمی‌گرداند
     * (تسک ۶۳۷: پیش از آن دو طرف دو ترتیب متفاوت داشتند و دقت پلاک ساختاراً صفر بود).
     *
     * ترتیب چاپِ قدیمیِ موتور («۱۲ ۳۴۵ ب ۶۷») هم پذیرفته و به قالب رسمی
     * برگردانده می‌شود، تا مقدارهای ذخیره‌شدهٔ پیش از تسک ۶۳۷ بی‌صدا رد نشوند.
     */
    private static function canonicalPlate(string $value): string
    {
        ...
        // قالب رسمی، با «ایران» یا بدون آن
        if (preg_match('/^([۰-۹]{2}) ('.$letters.') ([۰-۹]{3})(?: ایران)? ([۰-۹]{2})$/u', $plain, $m)) {
            return $m[1].' '.$m[2].' '.$m[3].' ایران '.$m[4];
        }

        // ترتیب چاپِ قدیمی موتور: دو رقم، سه رقم، حرف، دو رقم
        if (preg_match('/^([۰-۹]{2}) ([۰-۹]{3}) ('.$letters.') ([۰-۹]{2})$/u', $plain, $m)) {
    ...
    /**
     * پلاک: قالب رسمی «دو رقم، حرف، سه رقم، ایران، دو رقم».
     *
     * ورودی هر دو ترتیب را می‌پذیرد چون canonicalPlate ترتیب چاپِ قدیمیِ موتور
     * را هم به رسمی برمی‌گرداند؛ ولی چیزی که ذخیره و چاپ می‌شود همیشه رسمی است.
     */
    private static function validatePlate(string $value, string $label): ?string
    {
        ...
        $official = '/^[۰-۹]{2} (?:'.$letters.') [۰-۹]{3} ایران [۰-۹]{2}$/u';

        if (preg_match($official, $plain)) {
            return null;
        }

        return "«{$label}» با الگوی پلاک نمی‌خواند. الگوی درست: دو رقم، حرف، سه رقم، «ایران»،"
            .' دو رقم — مثل «۱۲ ب ۳۴۵ ایران ۶۷». حرف پلاک باید یکی از حروف مجاز باشد.';
    }

...
final class OcrText
{
     ...
     *
     * دو شرط جدا لازم است و شرط دوم را نمی‌شود از اولی نتیجه گرفت:
     *
     *   ۱) همهٔ نویسه‌ها از اسکریپت عربی (یا نیم‌فاصله) باشند — نه لاتین.
     *   ۲) توکن **حرف** داشته باشد و **رقم** نداشته باشد.
     *
     * شرط دوم بی‌سروصدا غایب بود و باگ بحرانی می‌ساخت: در یونیکد بلوک
     * `\p{Arabic}` ارقام فارسی (U+06F0…U+06F9) و عربی (U+0660…U+0669) را هم
     * در بر می‌گیرد، پس «۱۲۳۴۵» یک «واژهٔ فارسی» شمرده می‌شد و به‌عنوان نام
     * با اطمینان ۸۵ ذخیره می‌شد. آن اطمینان از آستانهٔ اعتماد بالاتر است، پس
     * تطابق نام بین مدارک `mismatch` می‌شد و پروندهٔ سالم خودکار رد می‌شد.
     *
     * نیم‌فاصله، همزه («ء») و اعراب (نویسه‌های ترکیبی مثل فتحه) عمداً مجازند:
     * اولی بخشی از املای فارسی است و دوتای دیگر حرف/نشانه‌اند نه رقم.
     */
    public static function isPersianWord(string $token, int $minLength = 2): bool
    {
        ...
        // دست‌کم یک حرف، و هیچ رقمی — «۱۲۳۴۵» و «۱۲ب» هر دو رد می‌شوند

 ...
 * مدارک پرونده (`storage/app/private/documents`)، نمونه‌های دیتاست، تصویرهای
 * تستیِ خودِ کاربر و بسته‌های خروجی هرگز در آن فهرست نیستند و این دستور به
 * آن‌ها کاری ندارد — تستش هم دقیقاً همین را تضمین می‌کند.
 *
 * یک استثنای عمدی: `dataset/evaluations` هم در فهرست است. آن پوشه نمونهٔ
 * دیتاست نیست؛ چند تصویرِ نمایشیِ «ارزیابی دقت» است که هیچ ردیفی در
 * `dataset_samples` ندارند و بعد از خوانده‌شدنِ عدد، دیگر به کار نمی‌آیند
 * (تسک ۷۲۶). نمونه‌های واقعی دیتاست در `dataset/YYYY-MM` می‌نشینند و
 * دست‌نخورده می‌مانند.
 *

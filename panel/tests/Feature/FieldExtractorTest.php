...
TXT;
    ...
    // ------------------------------------------------------------------
    // نامِ زیرِ برچسب — تسک ۶۶۳
    // ------------------------------------------------------------------

    /**
     * روی گواهینامه، «نام و نام خانوادگی» در یک سطر و مقدارش در سطر بعد است.
     *
     * متن زیر عیناً خروجی موتور روی نمونهٔ ۰۲۷ است. پیش از این تسک، هر دو
     * منبعِ readName روی سطر برچسب می‌افتادند و فیلد خالی می‌ماند — ۱۱ نمونه
     * از ۲۵ دقیقاً همین بودند، با نامی که OCR درست خوانده بود.
     */
    public function test_a_driving_license_name_printed_under_its_label_is_read(): void
    {
        $text = <<<'TXT'
تن ۳ نید نفک ۱
۳ گواهینامه رانندگی ۵ و
0 پایه اول م۱۳
شماره ملی ۸۷۱۶۲۲۷۰۰۱ پتستیر
نام و نام خانوادگی ۱
محمدجواد نوبختی - 7
تار یخ تولد ۱۳۸۷/۱۰/۰۴
1 تاریخ صدور ۱۴۰۵/۰۳/۲۳ مدت اعتبار ۱۰ سال
شماره گواهینامه ۰۳۱۵۸۳۲۳۸۰
TXT;

        $fields = $this->read('driving_license', $text);

        $this->assertSame('محمدجواد نوبختی', $fields['full_name']['normalized']);
        $this->assertGreaterThan(0.0, $fields['full_name']['confidence']);
    }

    /**
     * همان قاعده نباید روی کارت ملی فعال شود.
     *
     * آن‌جا «نام» و «نام خانوادگی» سطرهای پشت سر هم‌اند؛ اگر «نام» اجازه
     * داشته باشد به سطر بعد سُر بخورد، نام خانوادگی را به‌جای نام برمی‌دارد
     * و هر دو فیلد یک مقدار می‌گیرند.
     */
    public function test_a_national_card_first_name_never_slides_onto_the_last_name(): void
    {
        $text = <<<'TXT'
شماره ملی ۸۵۳۹۴۲۵۷۳۴۰
نام
نام خانوادگی هاشمی
سوت ۱۳۶۵/۰۳/۲۳
محمد
TXT;
        ...
        $fields = $this->read('national_card', $text);

        $this->assertNotSame('هاشمی', $fields['first_name']['normalized']);
        $this->assertSame('هاشمی', $fields['last_name']['normalized']);
    }
